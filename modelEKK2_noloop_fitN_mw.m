...
countries = {'AUT','BEL','CYP','CZE','DEU','DNK','ESP','EST','FIN', ...
    'GBR','GRC','HUN','IRL','ITA','LTU','LUX','LVA','MLT','NLD', ...
    'POL','PRT','SVK','SVN','SWE'};

figure(1)

x = D_NcDE(noFR)';
y = (D_RcDE_over_NcDE_FR(noFR) ./ D_NcDE(noFR))';
j = loglog(x,y,'.');
title('Data: Customers in Germany per French Exporters, Conditional');
xlabel('number exporting to Germany and elsewhere');
ylabel('customers in Germany per exporter, conditional');
pause
text(x,y*1.4,countries);
pause

figure(2)
h = loglog(NveccDE(noFR)',(M_RcDE_over_NcDE_FR(noFR)' ./ NveccDE(noFR))','.');
title('Model: Customers in Germany per French Exporters, Conditional');
xlabel('number exporting to Germany and elsewhere');
ylabel('customers in Germany per exporter, conditional');
pause
text(x,y*1.4,countries);
...
l = loglog(D_NcDE(noFR)',(D_RcDE_over_NcDE_FR(noFR) ./ D_NcDE(noFR))','kx', ...
    NveccDE(noFR)',(M_RcDE_over_NcDE_FR(noFR)' ./ NveccDE(noFR))','rs');
title('Customers in Germany per French Exporters, Conditional')
xlabel('number exporting to Germany and elsewhere');
ylabel('customers in Germany per exporter, conditional');
legend('Data','Model');
...
l = loglog(D_NcBE(noFR)',(D_RcBE_over_NcBE_FR(noFR) ./ D_NcBE(noFR))','kx', ...
    NveccBE(noFR)',(M_RcBE_over_NcBE_FR(noFR)' ./ NveccBE(noFR))','rs');
title('Customers in Belgium per French Exporters, Conditional')
